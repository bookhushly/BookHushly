 ...
 * Lets vendors enable airport pickup for a serviced apartment and set per-airport prices.
 * Prices are stored as JSONB in serviced_apartments.airport_prices.
 */
export function ApartmentAirportPricingTab({ apartmentId, apartment, onUpdate }) {
  const supabase = createClient();
  const [saving, setSaving] = useState(false);
  const [enabled, setEnabled] = useState(apartment?.airport_transfer_enabled ?? false);
  const [prices, setPrices] = useState(() => apartment?.airport_prices || {});
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {NIGERIAN_AIRPORTS.map((airport) => (
              <div
                key={airport.code}
                className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-purple-200 transition-colors"
              >
