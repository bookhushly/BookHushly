...
export function PushNotificationOptIn({ userId }) {
  const { isSupported, permission, isSubscribed, isLoading, optedOut, subscribe, unsubscribe } =
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-violet-100 bg-gradient-to-r from-violet-50 via-white to-purple-50 p-4">
      {/* Dismiss */}
      <button
      ...
      <div className="flex items-start gap-4 pr-8">
        {/* Icon */}
        <div className="shrink-0 w-10 h-10 rounded-xl bg-violet-600 flex items-center justify-center shadow-sm shadow-violet-600/20">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 leading-relaxed mb-3">
            Get instant push notifications for booking confirmations, payment updates, and more — even when the app is closed.
          </p>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={subscribe}
              disabled={isLoading}
              className="flex items-center gap-1.5 h-8 px-4 rounded-xl bg-violet-600 text-white text-xs font-semibold hover:bg-violet-700 transition-colors disabled:opacity-60 active:scale-95"
            >
            ...
            <button
              onClick={handleDismiss}
              className="h-8 px-3 rounded-xl text-gray-500 text-xs font-medium hover:bg-gray-100 transition-colors"
            >
              Maybe later
            </button>
          </div>
