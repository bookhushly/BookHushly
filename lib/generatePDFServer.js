 ...
 *
 * @param {Object}      booking        - Full booking object (with `.listing`)
 * @param {number}      ticketNumber   - 1-indexed position within this booking
 * @param {string}      ticketType     - Ticket tier name (e.g. "VIP")
 * @param {string}      baseUrl        - App base URL for QR code
 * @param {string}      templatePath   - File-system path to ticket.jpg template
...
export async function generateSingleTicketPDFServer(
  ...
  try {
    ...
    const ticketTypeDisplay = seatNumber ? `${ticketType}  ·  Seat ${seatNumber}` : ticketType;
    doc.setFontSize(placeholders.ticketType.fontSize);
    doc.setTextColor(...placeholders.ticketType.color);
    doc.text(ticketTypeDisplay, placeholders.ticketType.x, placeholders.ticketType.y);
...
export async function generateAllTicketPDFsServer(
  ...
  try {
    ...
    // seat_assignments is an ordered array matching individualTickets by index
