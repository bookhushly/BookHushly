...
export const emailTemplates = {
  ticket_confirmation: {
    ...
    template: (data) => `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <p>Your tickets for <strong>${data.event_title}</strong> have been confirmed.</p>
        <h3>Event Details</h3>
        ...
        <ul>
          ${Object.entries(data.ticket_details)
            .map(([name, quantity]) =>
              quantity > 0
                ? `<li>${name}: ${quantity} x ₦${(
                    quantity * data.ticket_prices[name]
                  ).toLocaleString()}</li>`
                : ""
            )
            .join("")}
        </ul>
        <p><strong>Total:</strong> ₦${data.total_amount.toLocaleString()}</p>
    ...
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 8px; overflow: hidden;">
      ...
      <div style="padding: 16px; background: #f9fafb;">
        ...
        <ul style="background: #ffffff; padding: 16px; border-radius: 6px; border: 1px solid #e5e7eb; margin: 0 0 16px; list-style: none;">
          ...
          <li style="color: #1f2937; font-size: 14px; margin-bottom: 8px;"><strong>Business Category:</strong> ${data.businessCategory}</li>
          <li style="color: #1f2937; font-size: 14px; margin-bottom: 8px;"><strong>Email:</strong> ${data.email}</li>
          <li style="color: #1f2937; font-size: 14px;"><strong>Phone:</strong> ${data.phone}</li>
        </ul>
  ...
  welcomeEmail: {
    ...
    template: (data) => `
      ...
      <p>Thanks for joining BookHushly. Start exploring now!</p>
    `,
  },
