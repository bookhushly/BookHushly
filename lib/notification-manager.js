    ...
    try {
      const emailTemplate = {
        ...
        data: {
          ...
          dashboard_url: `${process.env.NEXT_PUBLIC_APP_URL}/dashboard/vendor/payments`,
        },
      };
    ...
    try {
      ...
      switch (type) {
        case "admin_payment_received":
          message = `✅ Admin commission received: ${data.amount} ${data.currency}\nReference: ${data.transfer_reference}\nProvider: ${data.provider}`;
          color = "good";
          break;

        case "split_failure_alert":
          message = `⚠️ Payment split failure\nPayment ID: ${data.payment_id}\nError: ${data.error_message}\nTime: ${data.failure_time}`;
          color = "warning";
          break;

        case "critical_split_failure":
          message = `🔥 CRITICAL: Payment split permanent failure\nPayment ID: ${data.payment_id}\nError: ${data.error_message}\nAction: ${data.action_required}`;
          color = "danger";
          break;
      }
