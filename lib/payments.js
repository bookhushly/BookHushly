...
export const initializePaystackPayment = async (paymentData) => {
  try {
    ...
    if (!paymentData.metadata?.customer_id) {
      throw new Error("Missing required metadata: customer_id");
    }
    ...
    const metadata = {
      ...
      ...(paymentData.metadata?.hotel_booking_id && {
        hotel_booking_id: String(paymentData.metadata.hotel_booking_id),
      }),
      ...(paymentData.metadata?.apartment_booking_id && {
        apartment_booking_id: String(paymentData.metadata.apartment_booking_id),
      }),
    ...
    if (!data?.status || !data?.data || !data.data.authorization_url) {
      throw new Error("Invalid Paystack response: Missing required fields");
    }
    ...
    const paymentRecord = {
      reference: paymentData.reference,
      event_booking_id: metadata.event_booking_id || null,
      hotel_booking_id: metadata.hotel_booking_id || null,
      apartment_booking_id: metadata.apartment_booking_id || null,
...
// lib/payments.js
export const initializePaystackPayments = async (paymentData) => {
  try {
    // Validate environment variable
    if (!PAYSTACK_SECRET_KEY) {
      throw new Error("Paystack secret key not configured");
    ...
    // Flexible metadata validation - handle both booking and wallet deposits
    const isWalletDeposit =
      paymentData.metadata?.transaction_type === "wallet_deposit";
    const isBookingPayment = paymentData.metadata?.event_booking_id;

    if (!isWalletDeposit && !isBookingPayment) {
      throw new Error(
        "Invalid payment type: Must be either wallet deposit or booking payment"
      );
    }

    // For booking payments, require booking metadata
    if (isBookingPayment) {
      if (
        !paymentData.metadata?.event_booking_id ||
        ...
        throw new Error(
          "Missing required metadata for booking payment: event_booking_id or customer_id"
        );
      }
    }

    // For wallet deposits, require wallet metadata
    if (isWalletDeposit) {
      if (!paymentData.metadata?.user_id || !paymentData.metadata?.wallet_id) {
        throw new Error(
          "Missing required metadata for wallet deposit: user_id or wallet_id"
        );
      }
    }
    ...
    const metadata = {
      ...paymentData.metadata,
      payment_type: isWalletDeposit ? "wallet_deposit" : "booking_payment",
      ...
      ...(paymentData.metadata.user_id && {
        user_id: String(paymentData.metadata.user_id),
      }),
      ...(paymentData.metadata.wallet_id && {
        wallet_id: String(paymentData.metadata.wallet_id),
      }),
      ...(paymentData.metadata.transaction_id && {
        transaction_id: String(paymentData.metadata.transaction_id),
      }),
    };
...
export const initializeNOWPaymentsPayment = async (paymentData) => {
  try {
    if (!NOWPAYMENTS_API_KEY) {
      throw new Error("NOWPayments API key not configured");
    }
...
export const verifyNOWPaymentsPayment = async (paymentId) => {
  try {
    const response = await fetch(
...
export const createPaymentRecord = async (paymentData) => {
  try {
    ...
    const payload = {
      reference: paymentData.reference,
      ...(paymentData.event_booking_id && {
        event_booking_id: paymentData.event_booking_id,
      }),
      ...(paymentData.hotel_booking_id && {
        hotel_booking_id: paymentData.hotel_booking_id,
      }),
      ...(paymentData.apartment_booking_id && {
        apartment_booking_id: paymentData.apartment_booking_id,
      }),
...
export const getPaymentRecord = async (reference) => {
  try {
    const { data, error } = await supabase
      .from("payments")
      .select("*")
...
export const updatePaymentRecord = async (reference, updates) => {
  try {
    const { data, error } = await supabase
      .from("payments")
      .update({
