...
export const createVendorProfile = async (vendorData) => {
  console.log("🚀 createVendorProfile called");
  console.log("📦 vendorData received:", vendorData);
  const supabase = createClient();
  try {
    console.log("1️⃣ Starting profile creation...");

    // Check if vendorData is valid
    if (!vendorData) {
      throw new Error("vendorData is undefined or null");
    }

    if (!vendorData.user_id) {
      throw new Error("user_id is missing from vendorData");
    }

    console.log("2️⃣ Getting Supabase session...");
    const session = await supabase.auth.getSession();
    console.log("3️⃣ Session retrieved:", !!session.data?.session);

    const accessToken = session.data?.session?.access_token;
    console.log("4️⃣ Access token present:", !!accessToken);

    console.log("5️⃣ Preparing insert data...");
    const insertData = {
      ...
      nin: vendorData.nin || null,
      nin_first_name: vendorData.nin_first_name || null,
      nin_last_name: vendorData.nin_last_name || null,
      drivers_license: vendorData.drivers_license || null,
      tax_identification_number: vendorData.tax_identification_number || null,
      ...
      years_in_operation: vendorData.years_in_operation || null,
      website_url: vendorData.website_url || null,
      category_data: vendorData.category_data || {},
      approved: false,
      status: "reviewing",
    ...
    console.log("6️⃣ Insert data prepared:", {
      user_id: insertData.user_id,
      business_name: insertData.business_name,
      has_category_data: !!insertData.category_data,
    });

    console.log("7️⃣ Calling Supabase insert...");
    const { data, error } = await supabase
      .from("vendors")
      .insert(insertData)
      .select()
    ...
    console.log("8️⃣ Insert completed");
