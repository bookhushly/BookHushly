...
export async function getVendorHotels(vendorId) {
  ...
  const { data, error } = await supabase
    .from("hotels")
    .select("id, vendor_id, name, city, state, image_urls, description, amenities, price_per_night, created_at")
...
export async function getHotelById(hotelId) {
  ...
  const { data, error } = await supabase
    .from("hotels")
    .select("id, vendor_id, name, city, state, image_urls, description, amenities, price_per_night, checkout_policy, policies, security_deposit, security_deposit_notes, created_at")
    .eq("id", hotelId)
    .single();
...
export async function getHotelRoomTypes(hotelId) {
  ...
  const { data, error } = await supabase
    .from("hotel_room_types")
    .select("id, hotel_id, name, description, max_occupancy, base_price, size_sqm, amenities, image_urls, created_at")
    .eq("hotel_id", hotelId)
    .order("created_at", { ascending: false });
...
export const AMENITY_ICONS = [
  ...
  { value: "tv", label: "TV / DSTV", icon: "tv" },
  ...
  { value: "zap", label: "24h Power / Generator", icon: "zap" },
  { value: "battery-charging", label: "Inverter Backup", icon: "battery-charging" },
